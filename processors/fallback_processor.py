...
class SimpleFallbackProcessor:
    ...
    def process(self, file_path: str, page_indices: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        try:
            if file_extension == '.pdf':
                ...
                results = {"1": {"page_number": 1, "content": f"Unsupported file type: {file_extension}"}}
        except Exception as e:
            logger.error(f"Error processing document with fallback processor: {e}")
            results = {"1": {"page_number": 1, "content": f"Error processing document: {str(e)}"}}
        
        return results
    
    def _process_pdf(self, pdf_path: Path, page_indices: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"1": {"page_number": 1, "content": "PyMuPDF not available for PDF processing"}}
    ...
    def _process_text_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"1": {"page_number": 1, "content": f"Error reading text file: {str(e)}"}}
    ...
    def _process_doc_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"1": {"page_number": 1, "content": "python-docx not available for Word document processing"}}
        except Exception as e:
            logger.error(f"Error reading Word document: {e}")
            return {"1": {"page_number": 1, "content": f"Error reading Word document: {str(e)}"}}
    ...
    def save_results(self, results: Dict[str, Any], output_dir: str, base_name: str) -> None:
        ...
        with open(combined_md, "w", encoding="utf-8") as f:
            ...
            for page_num, page_data in sorted(results.items(), key=lambda x: int(x[0]) if x[0].isdigit() else x[0]):
