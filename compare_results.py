...
def extract_page_content(
    results: Dict[str, Dict[str, Any]], page_num: str
) -> Dict[str, str]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare results from different processors"
    )
    ...
    parser.add_argument(
        "--fix-combined",
        action="store_true",
        help="Fix combined results using OR logic",
    )
    ...
    if args.page:
        ...
        for page in all_pages:
            ...
            if args.fix_combined:
                ...
                with open(
                    md_file, "a" if md_file.exists() else "w", encoding="utf-8"
                ) as f:
