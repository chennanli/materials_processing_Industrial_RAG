...
from output_helpers import save_combined_results, save_processor_results
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("docling_ocr.log"), logging.StreamHandler()],
)
...
class BaseProcessor:
    ...
    def save_results(
        ...
        def _save_combined_results(combined_results, output_dir, base_name):
            ...
            """
            # Use our new helper function to save combined results
            save_combined_results(combined_results, output_dir, base_name)
...
try:
    ...
    logger.warning(
        "Camelot processor not available. Install with: pip install camelot-py[cv]"
    )
...
class DocumentProcessor:
    ...
    def _init_all_processors(self):
        ...
        if not self.processors:
            logger.warning(
                "No processors were available, using simple fallback processor"
            )
    ...
    def select_processors_for_document(
        ...
        if file_type == FileType.PDF:
            ...
            if ContentType.TEXT in content_types:
                if "lmstudio" in self.processors:
                    content_processors.setdefault(ContentType.TEXT, []).append(
                        "lmstudio"
                    )
            ...
            if ContentType.IMAGE in content_types:
                if "gemini" in self.processors:
                    content_processors.setdefault(ContentType.IMAGE, []).append(
                        "gemini"
                    )
    ...
    def process_document(
        ...
        for content_type, processors in processor_mapping.items():
            logger.info(
                f"Selected processors for {content_type}: {', '.join(processors)}"
            )
        ...
        if not any(processors for processors in processor_mapping.values()):
            logger.warning(
                "No processors selected based on content types, using all available processors"
            )
            ...
            for processor_name, processor in self.processors.items():
                ...
                if not any(
                    processor_name in processors
                    for processors in processor_mapping.values()
                ):
                    continue
    ...
    def _combine_results(
        self, processor_results: Dict[str, Any], processor_mapping: Dict[str, List[str]]
    ) -> Dict[str, Any]:
        ...
        for page_num in sorted(all_pages, key=lambda x: int(x)):
            combined_page = {"page_number": int(page_num), "content_sections": []}

            # Track what content types we've already added for this page
            # to avoid duplicates when using OR logic
            added_content_types = set()

            # First pass: Try to get content from preferred processors
            for content_type, processors in processor_mapping.items():
                content_added = False

                # Try each preferred processor first
                for processor_name in processors:
                    if (
                        processor_name in processor_results
                        and page_num in processor_results[processor_name]
                    ):
                        ...
                        if (
                            ...
                            and "tables" in processor_page
                            and processor_page["tables"]
                        ):
                            ...
                            content_added = True
                            added_content_types.add(ContentType.TABLE)

                        elif (
                            content_type == ContentType.TEXT
                            and "text" in processor_page
                            and processor_page["text"].strip()
                        ):
                            combined_page["content_sections"].append(
                                {
                            ...
                            content_added = True
                            added_content_types.add(ContentType.TEXT)

                        elif (
                            content_type == ContentType.IMAGE
                            and "images" in processor_page
                            and processor_page["images"]
                        ):
                            ...
                            content_added = True
                            added_content_types.add(ContentType.IMAGE)

                    # If we found content from this processor, move to next content type
                    if content_added:
                        break

            # Second pass: Try ANY processor for missing content types (OR logic)
            for content_type in [
                ContentType.TEXT,
                ContentType.TABLE,
                ContentType.IMAGE,
                ...
                if content_type in added_content_types:
                    continue

                # Try all processors for this content type
                for processor_name, results in processor_results.items():
                    if page_num in results:
                        processor_page = results[page_num]
            ...
            combined_page["content"] = self._format_combined_content(
                combined_page["content_sections"]
            )
    ...
    def _save_combined_results(
        self, combined_results: Dict[str, Any], output_dir: str, base_name: str
    ) -> None:
        """Save combined results.
        ...
        Args:
            ...
            base_name: Base name for output files
        """
        # Use the helper function to save results in the improved structure
        # This will also save in the original location for UI compatibility
        save_combined_results(combined_results, output_dir, base_name)
...
def main():
    ...
    # Page selection
    parser.add_argument(
        "--pages", help="Pages to process (e.g., '1-3,5,7-9'). Default: all pages"
    )

    # Output options
    parser.add_argument(
        "--output", help=f"Output directory for results. Default: {OUTPUT_DIR}"
    )
    ...
    if input_path.is_file():
        ...
        process_document_directory(
            input_path, args.processors, page_indices, output_dir
        )
    else:
...
def process_single_document(file_path, processors, page_indices, output_dir):
    ...
    results = doc_processor.process_document(
        file_path=str(file_path), page_indices=page_indices, output_dir=str(output_dir)
    )
